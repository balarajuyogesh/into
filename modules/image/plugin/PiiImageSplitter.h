 ...
 * equal size.) Unless the [randomize] flag is set to `true`, the
...
class PiiImageSplitter : public PiiDefaultOperation
{
  ...
  /**
   * Randomization flag. If this flag is set to `true`, the order of
   * emitted sub-images will be random. Otherwise, the upper left
   * corner will always become first. The default value is `false`.
   */
