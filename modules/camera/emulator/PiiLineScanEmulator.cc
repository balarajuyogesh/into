...
PiiLineScanEmulator::PiiLineScanEmulator() :
  ...
  _triggerMode(PiiCameraDriver::HardwareTrigger),
  ...
  _dGain(0.0),
  _iExposureTime(100),
  _iBaseExposureTime(100),
  _dTriggerRate(1.0),
  _dInputPulseFrequency(1000),
  _dOutputPulseFrequency(1000),
...
QVariant PiiLineScanEmulator::property(const char* name) const
{
  QVariant value;
  if (strncmp(name, "textureGenerator.", 17) == 0)
    {
      if (_pTextureGenerator != 0)
        value = _pTextureGenerator->property(name+17);
...
bool PiiLineScanEmulator::setProperty(const char* name, const QVariant& value)
{
  if (strncmp(name, "textureGenerator.", 17) == 0)
    {
      if (_pTextureGenerator != 0)
        return _pTextureGenerator->setProperty(name+17, value);
      else
        _mapGeneratorProperties[name+17] = value;
    }
  else
...
void PiiLineScanEmulator::initialize(const QString& /*cameraId*/)
{
  ...
  if (!_bOpen)
    {
      if (dataMap.contains("textureGeneratorName"))
        setTextureGeneratorName(dataMap.take("textureGeneratorName").toString());
...
{
  generateMultipliers();
  _leftTargetPoint = QPoint(_iLeftEdgeLimit/2,0);
  ...
  while (_bBufferingRunning)
    {
      if (_triggerMode == PiiCameraDriver::SoftwareTrigger)
        _triggerWaitCondition.wait();
      else if (_triggerMode == PiiCameraDriver::HardwareTrigger)
      ...
      while (_iCurrentLineIndex % _iHeight != 0)
        {
          if (newDefect() && _lstImages.size() > 0)
            {
...
bool PiiLineScanEmulator::setInputPulseFrequency(double freq)
{
  _dInputPulseFrequency = freq;
  _dOutputPulseFrequency = freq * _dTriggerRate;

  return true;
}

bool PiiLineScanEmulator::setTriggerRate(double triggerRate)
{
  _dTriggerRate = triggerRate;
  _dOutputPulseFrequency = _dInputPulseFrequency * triggerRate;
...
bool PiiLineScanEmulator::setTextureGeneratorName(const QString& textureGeneratorName)
{
  PiiTextureGenerator *pGenerator = PiiYdin::createResource<PiiTextureGenerator>(textureGeneratorName);
  if (pGenerator != 0)
    {
      delete _pTextureGenerator;
      _pTextureGenerator = pGenerator;
      _pTextureGenerator->setObjectName("textureGenerator");
      _pTextureGenerator->setParent(this);
    }
  else
    piiWarning(tr("TextureGenerator %1 is not available.").arg(textureGeneratorName));
...
QString PiiLineScanEmulator::textureGeneratorName() const
{
  if (_pTextureGenerator == 0)
    return "";
...
{
  return Pii::uniformRandom() <= _dTempProbability;
}
