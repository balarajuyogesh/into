      ...
      if (d->initMode == PiiClassification::SomSampleInit)
      ...
      else
        {
          ...
          this->setModels(PiiClassification::createRandomSampleSet<SampleSet>(d->iSizeX * d->iSizeY,
                                                                              iFeatures,
                                                                              Pii::minIn(sample, sample + iFeatures),
                                                                              Pii::maxIn(sample, sample + iFeatures)));
        }
    }
  ...
  if (iModels == 0)
    {
      if (d->initMode == PiiClassification::SomRandomInit)
        this->setModels(PiiClassification::createRandomSampleSet<SampleSet>(d->iSizeX * d->iSizeY,
                                                                            length,
                                                                            Pii::minIn(features, features + length),
                                                                            Pii::maxIn(features, features + length)));
      else
        {
          this->setModels(PiiSampleSet::create<SampleSet>(d->iSizeX * d->iSizeY,
                                                          length));
    ...
    case PiiClassification::SomBalancedAlgorithm:
      {
        ...
        double w = 1.0/(d->iIterationNumber+1);
