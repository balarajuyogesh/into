  ...
  while (d->lstClassifiers.size() < d->iMaxClassifiers)
    {
       ...
       * in individual sample weights seems to decrease numerical
       * accuracy. Therefore, 0.5 is intentionally used here. The
       * same factor is used with the ln(classCount-1) term in
       * SammeBoost.
       */
