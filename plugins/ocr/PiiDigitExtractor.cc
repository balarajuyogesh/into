...
PiiDigitExtractor::Data::Data() :
  iCombineYThreshold(6),
  iCombineXThreshold(2),
  dSizeThreshold(0.2),
  dLocationThreshold(0.6),
  bInverse(false)
{
}
...
template <class T> void PiiDigitExtractor::extractDigits(const PiiMatrix<T>& image)
{
  ...
  int iWindowSize = qMax(image.rows()/6, 8);
  int iLabelCount = 0;
  ...
  matThresholded = d->bInverse ?
    PiiImage::adaptiveThreshold(image, PiiImage::ThresholdFunction<int>(), 1.35, 0.0, iWindowSize, iWindowSize) :
    PiiImage::adaptiveThreshold(image, PiiImage::InverseThresholdFunction<int>(), 0.95, 0.0, iWindowSize, iWindowSize);
...
void PiiDigitExtractor::setInverse(bool inverse) { _d()->bInverse = inverse; }
bool PiiDigitExtractor::inverse() const { return _d()->bInverse; }
