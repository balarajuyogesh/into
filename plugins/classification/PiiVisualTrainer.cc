      ...
      for (int i=0; i<outputCount(); ++i)
        {
          ...
          if (inputAt(i+STATIC_INPUT_COUNT)->isConnected())
            {
          ...
          if (d->bFileNameConnected)
            {
              ...
              if (fileName.type() == PiiYdin::QStringType)
                pImage->setText("pii_filename", fileName.valueAs<QString>());
            }
          ...
          if (d->bLocationConnected)
            {
              ...
              d->iMainImageIndex++;
              pImage->setText("pii_id", QString::number(d->iMainImageIndex, 16));
...
void PiiVisualTrainer::updateInformation(QImagePtr image, const PiiMatrix<int>& location)
{
  ...
  // current main image.
  image->setText("pii_part_of", QString::number(d->iMainImageIndex, 16));
