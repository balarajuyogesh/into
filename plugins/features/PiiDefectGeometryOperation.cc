                                                                        ...
                                                                        float* mean, T* extremum)
{
  ...
  if (d->bDiscardExtrema)
    ...
    fMean = Pii::mean<float>(img);

  T extr = d->defectType == DarkDefect ? Pii::min(img) : Pii::max(img);
  if (extremum != 0)
    *extremum = extr;
  if (mean != 0)
    *mean = fMean;
...
{
  PII_D;
  float fVertStd = Pii::std<float>(vertDiff);
  float fHorzStd = Pii::std<float>(horzDiff);
  ...
  if (fVertStd == fHorzStd || // traps divzero
      qMin(fVertStd, fHorzStd) / qMax(fVertStd, fHorzStd) > 0.5) // ratio is less than 1/2
    {
      if (d->dGradientThreshold != 0)
        ...
        {
          vertThreshold = qMin(6*fVertStd, 0.35f*Pii::max(vertDiff));
          horzThreshold = qMin(6*fHorzStd, 0.35f*Pii::max(horzDiff));
        }
    }
