 ...
 * PiiBoundaryFinderOperation. Either @p boundaries or @p image must
 * always be connected.
 *
 * @in limits - limits of separate boundaries in the matrix read from
 * @p boundaries. This input is optional. If it is connected, the flow
 ...
 * @p boundaries and @p limits. If this input is connected, the
 ...
 * Consequently, the operation works as if @p SelectEveryNthPoint and
 * @p JoinAllShapes were set. All gray-level image types are accepted. 
 * If this input is connected, @p boundaries and @p limits must be
 ...
 * matches @p image. Usually, the direction matrix is received from an
 * edge detector. This input is optional and can be connected only if
 * @p image is connected.
 *
 * @outputs
 *
...
class PiiShapeContextOperation : public PiiDefaultOperation
{
  ...
  /**
   * The maximum number of points selected in @p SelectEveryNthPoint
