  ...
  else
    {
      switch (d->thresholdType)
        ...
        case RelativeToMeanThreshold:
          threshold = Pii::mean<double>(image) * d->dRelativeThreshold + d->dAbsoluteThreshold;
          break;
        case RelativeToMinThreshold:
          threshold = d->dRelativeThreshold * Pii::min(image) + d->dAbsoluteThreshold;
          break;
        case RelativeToMaxThreshold:
          threshold = d->dRelativeThreshold * Pii::max(image) + d->dAbsoluteThreshold;
          break;
        case MeanStdThreshold:
          {
            if (d->dRelativeThreshold != 0)
              {
                double mean;
                double stdev = Pii::std<double>(image, &mean);
                threshold = mean + d->dRelativeThreshold * stdev + d->dAbsoluteThreshold;
              }
            else
              threshold = Pii::mean<double>(image) + d->dAbsoluteThreshold;
          }
          break;
