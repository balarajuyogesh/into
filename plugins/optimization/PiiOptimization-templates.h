...
# error "Never use <PiiOptimization-templates.h> directly; include <PiiOptimization.h> instead."
#endif
...
namespace PiiOptimization
{
  ...
  typename Matrix::value_type assign(const Matrix& cost,
                                     PiiMatrix<int>* solution,
                                     PiiMatrix<typename Matrix::value_type>* duals) 

  {
    typedef typename Matrix::value_type T;
    PII_MATRIX_CHECK_SQUARE(cost);

    const int dim = cost.rows();
    ...
    PiiMatrix<int> matSolution(2, dim);
    int* rowsol = matSolution[0];
    int* colsol = matSolution[1];

    PiiMatrix<T> matDuals(2, dim);
    T* u = matDuals[0];
    T* v = matDuals[1];

    bool bUnassignedFound;
    int  i, imin, numfree = 0, prvnumfree, f, i0, k, freerow, *pred, *free;
    int  j, j1, j2, endofpath, last, low, up, *collist, *matches;
    T min, h, umin, usubmin, v2, *d;
    ...
    d = new T[dim];            // 'cost-distance' in augmenting path calculation.
        ...
        while (k < prvnumfree)
          {
            ...
            umin = cost(i,0) - v[0]; 
            j1 = 0; 
            usubmin = Pii::Numeric<T>::maxValue();
            for (j = 1; j < dim; j++) 
              {
                h = cost(i,j) - v[j];
                if (h < usubmin)
