      ...
      for (int j=0; j<d->iGroupSize; ++j)
        {
          ...
          // Shift all connected inputs and emit their
          // incoming objects.
          ...
          // default value is there.
            ...
            emitObject(d->lstDefaultValues[d->defaultValueMode == SameDefaultsForAllGroups ?
                                           j :
                                           iFirstInGroup + j],
                       j+1);
        }
...
void PiiPisoOperation::setDefaultValue(const QVariantMap& map)
{
  ...
  PiiVariant defaultObject = map.value("defaultValue", QVariant()).value<PiiVariant>();
  ...
  if (index < d->lstDefaultValues.size())
  ...
  else
    {
      ...
      for ( int i=d->lstDefaultValues.size(); i<index; i++)
        d->lstDefaultValues.append(PiiVariant());
...
void PiiPisoController::InputGroup::shift()
{
  ...
  if (_lstActiveGroups.size() == 0)
    {
      ...
      switch (_iTagMask)
        ...
        case PauseTag:
          return PausedState;
        case ReconfigurationTag:
          setPropertySetName(_lstGroups[0][0]->firstObject().valueAs<QString>());
...
void PiiPisoOperation::setDefaultValues(const PiiVariantList& defaultValues) { _d()->lstDefaultValues = defaultValues; }
PiiVariantList PiiPisoOperation::defaultValues() const { return _d()->lstDefaultValues; }
