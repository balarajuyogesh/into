...
void PiiDefaultFlowController::SyncGroup::shiftInputs()
{
  for (int i=size(); i--; )
    at(i)->shift();
}
      ...
      for (int i=tmpRelations.size(); i--;)
        {
          ...
          if (bRoot)
            {
              // Go through the relations list and shift all children
              // of the root group.
  ...
  if (d->vecActiveSyncGroups.size() == 0)
    {
      ...
      if (uiTagType == PiiYdin::StopTagType)
        return FinishedState;
      else if (uiTagType == PiiYdin::PauseTagType)
        return PausedState;
      ...
      else // if (uiTagType == PiiYdin::ReconfigureTagType)
        {
          setPropertySetName(d->vecSyncGroups[0]->at(0)->firstObject().valueAs<QString>());
          return ReconfigurableState;
        }
    }
...
bool PiiDefaultFlowController::hasSyncEvents() const
{
  return _d()->vecSyncEvents.size() > 0;
}
