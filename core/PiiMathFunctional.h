...
namespace Pii
{
  ...
  /**
   * An adaptable unary function for calculating the base 2 logarithm.
   */
  template <class T> struct Log2 : public Pii::UnaryFunction<T,typename RealReturnValue<T>::Type>
  {
    typename RealReturnValue<T>::Type operator() (const T& value) const { return log2(value); }
  };

  /**
   * An adaptable unary function for calculating the base 10 logarithm.
   */
  template <class T> struct Log10 : public Pii::UnaryFunction<T,typename RealReturnValue<T>::Type>
  {
    typename RealReturnValue<T>::Type operator() (const T& value) const { return log10(value); }
  };
  ...
  public:
    ...
    template <class V> V mean() const { return _iInvocations != 0 ? V(_accumulator) / _iInvocations : 0; }

    /**
     * Calculate the current mean value by dividing the accumulator by
     * the number of invocations.
     */
    U mean() const { return mean<U>(); }
