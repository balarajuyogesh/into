 ...
 * @endcode
 *
 * @par Mapping HTTP request to function calls
 *
 ...
 *
 * @par Channels and call-back functions
 *
 ...
 * @li /channels/
 * @li /channels/new
 * @li /channels/channel-id
 * @li /channels/channel-id/connect
 * @li /channels/channel-id/disconnect
 * @li /channels/channel-id/sources
 * @li /channels/channel-id/delete
 *
...
public:
   ...
   *
   * @lip AccessFromMainThread - the object/function can only be
   ...
   * GUI-related tasks.
   *
   * @lip AccessFromAnyThread - the object/function can be accessed
   * from any thread, but synchronized wrt each other.
   *
   * @lip AccessConcurrently - the object/function can be accessed
   ...
   * calls. The default value is @p AccessFromAnyThread.
   *
   * @note If a QObjectServer is not created in the main thread (e.g. 
   * via PiiInstantiableObjectServer), it must be moved to the main
   * thread if the access mode is @p AccessFromMainThread. Otherwise,
   ...
   * @p AccessFromMainThread automatically moves this object to the
   * main thread. This move cannot be undone. Thus, once the safety
   * level is set to @p AccessFromMainThread, it can no longer be
   * changed.
   */
  ...
  /**
   * Sets the safety level of an individual function, identified by @a
  ...
  /**
   * Adds @a function to the server's interface as @a name. 
  ...
  /**
   * Registers a call-back function with the given @a signature. The
   ...
   *
   * @return @p true if successful, @p false if @a signature is
   * already registered or ill-formatted.
   */
  bool addCallback(const QString& signature);
  /**
   * Removes a call-back with the given @a signature.
   */
   ...
   *
   * Invokes the call-back function called @a function with the given
...
protected:
  ...
  public:
    virtual ~Channel();
    /**
     * Puts @a data from the source identified by @a sourceId to the
     * output queue. Returns @p true if successful, @p false if the
  ...
  /**
   * Invokes @a function with @a params and returns its return value. 
   ...
   * implementation throws @p NotFoundStatus if the function does not
   * exist, and @p BadRequestStatus if an overload matching @a params
   * cannot be found.
   *
  ...
  /**
   * Connects the pushable the source identified by @a sourceId to @a
   * channel. The default implementation assumes that @a sourceId is
   ...
   * sources to channels. Whenever new pushable data from @a
   * sourceUri is available, it should be pushed to all channels to
   * which the source is connected.
   *
   * @exception PiiHttpException& if something goes wrong. The default
   * implementation throws an exception with @p NotFoundStatus if @a
   * sourceUri isn't a valid URI for a signal.
   */
  virtual void connectToChannel(Channel* channel, const QString& sourceId);
  
  /**
   * Disconnects the pushable the source identified by @a sourceId
   * from @a channel. The default implementation assumes that @a
   ...
   * more data from @a sourceId must be pushed to the channel.
   *
   * @exception PiiHttpException& if something goes wrong. The default
   * implementation throws an exception with @p NotFoundStatus if @a
   * sourceUri isn't a valid URI for a signal.
  ...
  /**
   * Informs subclasses that @a channel is about to be deleted. The
