...
void* PiiTypelessMatrix::appendRow(size_t bytesPerRow)
{
...
void* PiiTypelessMatrix::insertRow(int index, size_t bytesPerRow)
{
...
void PiiTypelessMatrix::removeRow(int index, size_t bytesPerRow)
{
  if (index != -1)
...
void PiiTypelessMatrix::removeRows(int index, int cnt, size_t bytesPerRow)
{
...
void PiiTypelessMatrix::removeColumn(int index, size_t bytesPerItem)
{
  removeColumns(index, 1, bytesPerItem);
}

void PiiTypelessMatrix::removeColumns(int index, int cnt, size_t bytesPerItem)
{
  if (index != -1)
    {
      size_t iRowEndBytes = (d->iColumns - index - cnt) * bytesPerItem;
...
void* PiiTypelessMatrix::appendColumn(size_t bytesPerItem)
{
  int iRows = d->iRows;
...
void* PiiTypelessMatrix::insertColumn(int index, size_t bytesPerItem)
{
  if (index == -1 || index == d->iColumns)
    return appendColumn(bytesPerItem);

  size_t iRowEndBytes = (d->iColumns - index) * bytesPerItem;
  size_t iBytesPerRow = (d->iColumns + 1) * bytesPerItem;
  ...
  if (d->iRefCount != 1 ||
      ...
      iBytesPerRow > d->iStride)
    {
      PiiMatrixData* pData = PiiMatrixData::createUninitializedData(qMax(d->iRows, d->iCapacity),
                                                                    ...
                                                                    iBytesPerRow);
      pData->iRows = d->iRows;
      size_t iRowStartBytes = index * bytesPerItem;
...
void PiiTypelessMatrix::cloneAndReplaceData(int capacity, size_t bytesPerRow)
{
...
void PiiTypelessMatrix::reserve(int rows, size_t bytesPerRow)
{
...
void PiiTypelessMatrix::resize(int rows, int columns, size_t bytesPerItem)
{
  if (rows == d->iRows && columns == d->iColumns)
    return;

  size_t iBytesPerRow = bytesPerItem * columns;
  ...
  // to reallocate.
  else
    {
      ...
      size_t iMinBytes = qMin(iBytesPerRow, bytesPerItem * d->iColumns);
