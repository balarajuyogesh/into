...
namespace Pii
{
  ...
  template <class U, class Matrix> PiiMatrix<U> sum(const Matrix& mat, MatrixDirection direction)
  {
    ...
    for (int r=1; r<iRows; ++r)
      {
        ...
        pTarget = result[r+int(mode)] + int(mode);
                                                               ...
                                                               CumulativeSumMode mode)
  {
    ...
    PiiMatrix<U> matResult(PiiMatrix<U>::uninitialized(matrix.rows() + int(mode), matrix.columns() + int(mode)));
    if (mode == ZeroBorderCumulativeSum)
      {
        Pii::fillN(matResult.rowBegin(0), matResult.columns(), U(0));
        Pii::fillN(matResult.columnBegin(0), matResult.rows(), U(0));
      }
    if (matrix.isEmpty())
      return matResult;
  ...
  template <class U, class Matrix> U var(const Matrix& mat, U* average)
  {
    U avg = mean<U>(mat);
    U sum(0);
    for (typename Matrix::const_iterator i = mat.begin(); i != mat.end(); ++i)
      sum += square(U(*i) - avg);
    if (average != 0)
      *average = avg;
                                      ...
                                      int* maxR, int* maxC)
  {
    if (mat.isEmpty())
      return;
    typename Matrix::const_iterator minIt = mat.begin(), maxIt = mat.begin();
    for (typename Matrix::const_iterator it = mat.begin(); it != mat.end(); ++it)
      {
        if (*it < *minIt)
          minIt = it;
        else if (*it > *maxIt)
    ...
    *minimum = *minIt;
    *maximum = *maxIt;
    if (minR) *minR = minIt.row();
    if (minC) *minC = minIt.column();
    if (maxR) *maxR = maxIt.row();
    if (maxC) *maxC = maxIt.column();
  }
        ...
        if (overElements != 0)
          {
            ...
            T value = medianN(lastElements, overElements);
...
#define MEDIAN_SORT(a,b) { if ((a)>(b)) MEDIAN_ELEM_SWAP((a),(b)); }
  ...
  template <class Iterator> typename std::iterator_traits<Iterator>::value_type medianN(Iterator data, int len)
  {
    typedef typename std::iterator_traits<Iterator>::value_type T;
